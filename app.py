            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix='.pdf') as tmp_pdf, \
                ...
                with btn_col1:
                    with open(tmp_pdf.name, "rb") as f:
                        st.download_button("下载PDF文件", f, file_name=f"产品研发部-综合业务组周报汇总-{date_str}.pdf", use_container_width=True, key="pdf")
                with btn_col2:
                    with open(tmp_docx.name, "rb") as f:
                        st.download_button("下载Word文件", f, file_name=f"产品研发部-综合业务组周报汇总-{date_str}.docx", use_container_width=True, key="word")
